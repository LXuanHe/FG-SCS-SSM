...
#     idealization=NO_IDEALIZATION, symmetric=False, thicknessType=UNIFORM, 
#     poissonDefinition=DEFAULT, thicknessModulus=None, temperature=POINTWISE, 
#     nTemp=21, useDensity=OFF, integrationRule=SIMPSON, layup=(sectionLayer1,sectionLayer2, ))
...
############################################
#mdb.saveAs(pathName='Nonlinear_Newton')      
nodelist=p.nodes
...
m.DisplacementBC(amplitude=UNSET,createStepName='Initial',distributionType=UNIFORM,
    fieldName='',fixed=OFF,localCsys=None,name='BC-1',region=r.instances[gg].sets['Set_Edge_shellbot_wgjj'],u1=0.0,
    u2=0.0,u3=0.0,ur1=0.0,ur2=0.0,ur3=0.0)              #在Initial里添加约束(底部)   #约束与荷载的坐标系要一致
    
m.DisplacementBC(amplitude=UNSET,createStepName='Initial',distributionType=UNIFORM,
    fieldName='',fixed=OFF,localCsys=None,name='BC-2',region=r.sets['Set_Rpt_wgjj'],u1=0.0,
    u2=0.0,u3=0.0,ur1=0.0,ur2=0.0,ur3=0.0)            #在Initial里添加约束(顶部),注意：一定要在耦合参考点加约束，否则无法得到该点的支座反力
                                                      #猜想：若下面的历史输出数据不是支座反力'RT'，而是反力'RF'，那么该约束是否就可以加在
                                                      # r.instances[gg].sets['Set_Edge_shellbot_wgjj']，而不用加在 r.sets['Set_Rpt_wgjj']上面
m.HistoryOutputRequest(name='H-Output-2', createStepName='Step-1', 
    variables=('RT', ), region=r.sets['Set_Rpt_wgjj'], sectionPoints=DEFAULT, rebar=EXCLUDE)   #设定历史输出数据为参考点的支座反力 'RT' 
m.fieldOutputRequests['F-Output-1'].setValues(variables=('S', 'U', 'RF', 'NT', 'TEMP'))
...
######模型质量输出######
zd=mdb.models[modelname].rootAssembly.getMassProperties()  ##重量处有些问题
weight=zd['mass']
weight2=weight*1000.0
outfile=open('output1_W.txt','w')
...
#         multiprocessingMode=DEFAULT,name='a1',nodalOutputPrecision=SINGLE,
#         numCpus=2,numDomains=4,queue=None,scratch='',type=ANALYSIS,
#         userSubroutine='',waitHours=0,waitMinutes=0)
#         
# mdb.jobs['a1'].writeInput(consistencyChecking=OFF)   #运行job，写到INP文件同时不进行数据检查
# 
# f=open('a1.inp')
# out=open('am1.inp','w')
# 
# for i in range(0,996549):
#     line=f.readline()  #根据循环依序从file 文件中读取行
#     if line=='' or line=='\n':
#         break
...
#     else:
#         out.write(line)
#         out.write('*Node File, global=yes'+'\n')
#         out.write('u,'+'\n')
# 
# f.close()
# out.close()
# 
# mdb.JobFromInputFile(name='am11',inputFileName='am1.inp',        
#     type=ANALYSIS,atTime=None,waitMinutes=0,waitHours=0,queue=None,
#     memory=90,memoryUnits=PERCENTAGE,getMemoryFromAnalysis=True,
#     explicitPrecision=SINGLE,nodalOutputPrecision=SINGLE,userSubroutine='',
#     scratch='',parallelizationMethodExplicit=DOMAIN,numCpus=2,numDomains=4,
#     activateLoadBalancing=False,multiprocessingMode=DEFAULT)

mdb.Job(name='am11', model=modelname, description='', type=ANALYSIS, atTime=None, 
    waitMinutes=0, waitHours=0, queue=None, memory=90, memoryUnits=PERCENTAGE, 
    getMemoryFromAnalysis=True, explicitPrecision=SINGLE, 
    nodalOutputPrecision=SINGLE, echoPrint=OFF, modelPrint=OFF, 
    contactPrint=OFF, historyPrint=OFF, userSubroutine='', scratch='', 
    resultsFormat=ODB, multiprocessingMode=DEFAULT, numCpus=8, numDomains=16, numGPUs=0)

mdb.saveAs(pathName='Nonlinear_Newton1')     
    
mdb.jobs['am11'].submit(consistencyChecking=OFF)

mdb.jobs['am11'].waitForCompletion()


# 对当前视口中的输出数据库进行操作
odb=session.openOdb(name='E:/HSM_AK_FGMshell/HSM_lightweight/am11.odb')
xy0 = xyPlot.XYDataFromHistory(odb=odb, 
    outputVariableName='Reaction force: RF3 PI: rootAssembly Node 1 in NSET SET_RPT_WGJJ', 
    steps=('Step-1', ), suppressQuery=True)
xy1 = abs(xy0)
xy1=xy1[-1]

outfile=open('output1_F.txt','w')
outfile.write('wenduhezai='+str(xy1)+'\n')
outfile.close()
