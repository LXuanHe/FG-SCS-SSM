...
######## 
fd=file('E:/HSM_AK_FGMshell/HSM_lightweight/inputabaqus.txt','r')   #r表示以只读方式打开文件  
HOU=float(fd.readlines()[0])
fd.close()
fd=file('E:/HSM_AK_FGMshell/HSM_lightweight/inputabaqus.txt','r')
...
Tem=300.0               #始温
TemO=Tem+upTemO         #外表面（金属层）终温
TemI=Tem+upTemI         #内表面（陶瓷层）终温
...
ptname='p1'
modelname='aa'
computmethod='Explicit'


##########材料物参#############
posongb=0.288
Iposongb=posongb     #内表面（陶瓷）泊松比
Oposongb=posongb     #外表面（金属）泊松比

Imidu=3.97e-9         #内表面（陶瓷）密度，g/mm^3
Omidu=4.429e-9       #外表面（金属）密度
...
IE0=244.27e3         #内表面（陶瓷）弹模,单位：N/mm^2 = MPa
IE=0.0
IE1=-1.3707e-9
IE2=1.21393e-12
IE3=-3.6814e-16

IA0=12.768e-6        #内表面（陶瓷）线胀，单位：1/K
IA=0.0
IA1=-1.49e-3
IA2=0.1e-5
IA3=-0.6775e-11

IK0=1.7              #内表面（陶瓷）热导率，单位：W/m*K，瓦/米·度
IK=0.0
IK1=0.1276e-3
IK2=0.66485e-5
IK3=0.0

OE0=122.56e3         #外表面（金属）弹模
OE=0.0
OE1=-4.5866e-10
OE2=0.0
OE3=0.0

OA0=7.5788e-6        #外表面（金属）线胀
OA=0.0
OA1=6.5e-4
OA2=0.31467e-6
OA3=0.0

OK0=1.20947          #外表面（金属）热导率
OK=0.0
OK1=0.0139375
OK2=0.0
OK3=0.0
...
mdb.models[modelname].materials['Ti'].Density(table=((Omidu, ), ))
...
mdb.models[modelname].Material(name='ZrO')            #陶瓷，ZrO2
mdb.models[modelname].materials['ZrO'].Density(table=((Imidu, ), ))
mdb.models[modelname].materials['ZrO'].Elastic(temperatureDependency=ON,
...
#mdb.models[modelname].materials['ZrO'].Plastic(table=((YS, 0.0), ))  #陶瓷不考虑塑性
mdb.models[modelname].materials['ZrO'].Expansion(temperatureDependency=ON, 
...
mdb.models[modelname].materials['ZrO'].Conductivity(temperatureDependency=ON,
...
#1到20，内层到外层, 设定单层温度分布点（InPt个温度点，与积分点数相同；若温度点数小于积分点数，则积分点根据温度点进行线性插值）
...
m.HomogeneousShellSection(thickness=tw,material='ZrO',name='Shell_jintiao_wgjj-'+ptname,
...
############################################
#mdb.saveAs(pathName='Nonlinear_Explicit')      
nodelist=p.nodes
...
if computmethod=='Explicit':        #显示动力学分析 
    loadValue=-H*0.01
    m.ExplicitDynamicsStep(name='Step-1', previous='Initial', timePeriod=computTime)   #创建分析步Step-1，显示动力
    m.ExplicitDynamicsStep(name='Step-2', previous='Step-1', timePeriod=computTime)    #创建分析步Step-2，显示动力
    m.ExplicitDynamicsStep(name='Step-3', previous='Step-2', timePeriod=computTime)    #创建分析步Step-2，显示动力
    ...
    m.fieldOutputRequests['F-Output-1'].setValues(numIntervals=40, timeMarks=ON)
    m.TabularAmplitude(name='Amp-1', timeSpan=STEP, smooth=SOLVER_DEFAULT, data=((0.0,0.0), (computTime,1.0)))   #在工具栏创建一个类型为表的幅值曲线
    m.DisplacementBC(name='disp', createStepName='Step-3', fieldName='', 
        fixed=OFF, localCsys=None, region=r.sets['Set_Rpt_wgjj'], amplitude='Amp-1', 
        u1=UNSET, u2=UNSET, u3=loadValue, ur1=UNSET, ur2=UNSET, ur3=UNSET, )      #极限荷载
elif computmethod=='Linear_Buck':
    m.StaticStep(name='Step-1', previous='Initial')  #, initialInc=0.05, nlgeom=ON
    m.BuckleStep(name='Step-2', previous='Step-1', numEigen=1, 
...
m.DisplacementBC(name='BC-1', createStepName='Initial', region=r.instances[gg].sets['Set_Edge_shellbot_wgjj'], 
    distributionType=UNIFORM, fieldName='', fixed=OFF, localCsys=None, amplitude=UNSET, 
    u1=0.0, u2=0.0, u3=0.0, ur1=0.0, ur2=0.0, ur3=0.0)              #添加约束(底部)   #约束与荷载的坐标系要一致    
m.DisplacementBC(name='BC-2', createStepName='Initial', region=r.sets['Set_Rpt_wgjj'], 
    distributionType=UNIFORM, fieldName='', fixed=OFF, localCsys=None, amplitude=UNSET,
...
mdb.Job(atTime=None,contactPrint=OFF,description='',echoPrint=OFF,
        ...
        multiprocessingMode=DEFAULT,name='a3',nodalOutputPrecision=SINGLE,
        numCpus=8,numDomains=16,queue=None,scratch='',type=ANALYSIS,
        userSubroutine='',waitHours=0,waitMinutes=0)
        
mdb.jobs['a3'].writeInput(consistencyChecking=OFF)   #运行job，写到INP文件同时不进行数据检查

f=open('a3.inp')
out=open('am3.inp','w')

for i in range(0,996549):
    line=f.readline()
    if line=='' or line=='\n':
        break
    if line!='** ----------------------------------------------------------------'+'\n':
...
mdb.JobFromInputFile(name='am33',inputFileName='am3.inp',
    type=ANALYSIS,atTime=None,waitMinutes=0,waitHours=0,queue=None,
    memory=90,memoryUnits=PERCENTAGE,getMemoryFromAnalysis=True,
    explicitPrecision=SINGLE,nodalOutputPrecision=SINGLE,
    userSubroutine='',scratch='',parallelizationMethodExplicit=DOMAIN,
    numDomains=16,activateLoadBalancing=False,multiprocessingMode=DEFAULT,
    numCpus=8)
mdb.saveAs(pathName='Nonlinear_Explicit3')     
    
mdb.jobs['am33'].submit(consistencyChecking=OFF)

mdb.jobs['am33'].waitForCompletion()


# 对当前视口中的输出数据库进行操作
odb=session.openOdb(name='E:/HSM_AK_FGMshell/HSM_lightweight/am33.odb')

# 搜索最大RF(压溃荷载)
maxValue = None
RFOutputExists = FALSE
for step in odb.steps.values():
    for frame in step.frames:
        try: 
            ...
            continue
        for RFValue in RF.values:
            if (not maxValue or
                    RFValue.magnitude > maxValue.magnitude):
                    maxValue = RFValue
                    maxStep, maxFrame = step, frame  

# 如果odb文件中没有输出应力结果，则抛出异常。
if not RFOutputExists:
    raise '该odb文件不包含应力结果的输出。'

# 输出最大RF的详细信息
print '搜索到的最大RF %E 位于：' % maxValue.magnitude                   
print '  分析步:             ', maxStep.name 
print '  帧:                 ', maxFrame.frameId                  

outfile=open('output3_F.txt','w')
outfile.write('jixianli='+str(maxValue.magnitude)+'\n')
outfile.close()
